...
namespace cynlr{
    ...
    >
    struct task_filter_threshold<task_mode::debug, DataPoolType, FilterWeightDataType, ThresholdDataType, FilterWindowRadius> {
        ...
        auto loop() -> std::expected<task_response, task_error> {
            if(resource.receive(value)){
                ...
                if (index == index_end) {
                    ...
                    Log(std::format("RX Out: ({})", join_str<DataType>(thresholded_data)));
