...
int main(){
    ...
    using data_pool_type = data_pool<data_pool_mode, u8, csize<32>>;
    data_pool_type pool;
    ...
    task_data_generator<task_mode::testing, data_pool_mode, u8, 2u, 32u> task1(pool, tx_data);
    task_filter_threshold<task_mode::realtime, data_pool_mode, u8, f32, u8, 175u, 32u, 4u> task2(pool, filter_weights);
    ...
    usize print_iterations = roundUp(tx_data.size(), 32);
...
// constexpr size_t roundUp(size_t dividend, size_t divisor) {
//     return (dividend + divisor - 1) / divisor;
// }

// constexpr size_t TX_BATCH_SIZE = 2;
// constexpr size_t TX_DATA_SIZE = 646;
// constexpr size_t TX_NUM_ITERATIONS = roundUp(TX_DATA_SIZE, TX_BATCH_SIZE);
...
//     constexpr size_t PRINT_ITERATIONS = roundUp(TX_DATA_SIZE, RX_BATCH_SIZE);
